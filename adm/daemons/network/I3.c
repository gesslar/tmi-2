...
// Brainstorm May 96 - Added some more support for I3 channels.
*/
...
// check for security.
    ...
    if( fname == "chan" ) fname="channel";
    if( debug_level & I3_DEBUG_LST ) 
	debug_stuff( "Calling "+I3_MODULES+fname+" "+func );
...
// == "/adm/daemons/network/I3/" check here.
    ...
    if(!query_network_name(mud) && mud != "global" && mud != 0
	&& mud != ROUTER_NAME ){
 return 0;
}
    if( mud != ROUTER_NAME )
...
void process_chanlist_reply(mixed *info){
    ...
    if(!chanlist) chanlist = ([]); // Brainstorm TEST HACK! was /* */

    kys = keys(chanlist);

    // At the moment the router only sends complete lists
    // so this might actually work.

    if( mapp( info[7] ) )	
    for( i=0;i<sizeof(kys); i++ )
	if( chanlist[kys[i]] && !info[7][kys[i]]) {
		map_delete(chanlist, kys[i]);
                CHANNELS_D->delete_channel( kys[i] );
                if(debug_level & I3_DEBUG_LST) debug_stuff("Removing Channel \""+kys[i]+"\"\n" );
        }
    ...
    // Bootflag == 1 means the CHANNELS_D shall tune us in to
    // the channels users are listening to.
    CHANNELS_D->add_i3channels( chanlist, bootflag );
    bootflag=0;
...
void process_error(mixed *info){
    ...
    // Lets try to get a basic error handling
    switch( info[6] ) {
	case "unk-channel": // Unkown Channel, lets remove it from CHANNELS_D
		error=info[8];
		if( (string)error[0] == PRT_CHANNEL_LISTEN ) {
		  if(debug_level & I3_DEBUG_LST) debug_stuff("Removing Channel \""+error[6]+"\"\n" );
	  	  CHANNELS_D->delete_channel( error[6] );
		}
	        break;
	default:
		break;
...
void create(){ 
    //tc("I3_SAVE_FILE: "+I3_SAVE_FILE+" exists?: "+file_size(I3_SAVE_FILE));
    if(sizeof(get_dir(I3_SAVE_FILE))) restore_object(I3_SAVE_FILE);

    // Set boot flag on create, delete it on channel list processing. 
    bootflag=1;
